...
import (
	geeTracer "github.com/gee-coder/gee/tracer"
...
func Tracer(serviceName string, samplerConfig *config.SamplerConfig, reporter *config.ReporterConfig, options ...config.Option) MiddlewareFunc {
	return func(next HandlerFunc) HandlerFunc {
		return func(ctx *Context) {
			...
			tracer, closer, spanContext, _ := geeTracer.CreateTracerHeader(serviceName, ctx.R.Header, samplerConfig, reporter, options...)
			defer closer.Close()
			// 生成依赖关系，并新建一个 span、
